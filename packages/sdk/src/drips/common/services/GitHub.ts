...
export default class GitHub {
  ...
  public async verifyFundingJson(owner: string, repo: string): Promise<void> {
    ...
    const fileContent = Buffer.from((data as any).content, "base64").toString(
      "utf-8",
    );
    ...
    const fundingJsonOwner =
      fundingJson.drips?.[
        network.name === "homestead" ? "ethereum" : network.name
      ].ownedBy;

    const { address: expectedOwner } = await get();

    if (fundingJsonOwner.toLowerCase() !== expectedOwner?.toLowerCase()) {
      throw new Error(
        "Invalid FUNDING.json file. Does it have the correct Ethereum address?",
      );
    }
  }
}
