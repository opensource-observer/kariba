...
import type {
  ...
  LatestVersion,
  Parser,
} from "@efstajas/versioned-parser";
import type {
  AccountId,
  IpfsHash,
  MetadataKeyValue,
...
import { fetchIpfs as ipfsFetch } from "&/drips/common/utils/ipfs.js";
  ...
  implements IMetadataManager<TParser>
{
  ...
  private readonly _emitMetadataFunc:
    | typeof executeNftDriverWriteMethod
    | undefined;
   ...
   */
  public abstract buildAccountMetadata(
    context: unknown,
   ...
   */
  public abstract fetchMetadataHashByAccountId(
    accountId: AccountId,
  ): Promise<string | null>;
  ...
  public async fetchAccountMetadata(
    ...
    let accountMetadataRes: Awaited<
      ReturnType<typeof MetadataManagerBase.prototype.fetchIpfs>
    >;
   ...
   */
  public async pinAccountMetadata(
    data: LatestVersion<TParser>,
  ): Promise<IpfsHash> {
  ...
  public async updateAccountMetadata<T extends z.ZodType>(
    ...
    const currentOnChainHash =
      await this.fetchMetadataHashByAccountId(accountId);
  ...
  private async emitAccountMetadata(newHash: IpfsHash, accountId: AccountId) {
    assert(
      this._emitMetadataFunc,
      "emitAccountMetadata called without emitMetadataFunc",
    );
